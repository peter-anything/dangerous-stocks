...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for stock_review in stock_reviews:
            if stock_review.code == '603690':
                print(stock_review.code)
            if pre_st_review:
        ...
        for candi_stock in candi_stocks:
            industry_item = industry_map.get(candi_stock.industry, IndustryItem(candi_stock.industry, []))
            industry_item.stocks.append(candi_stock)
            industry_map[candi_stock.industry] = industry_item

        industry_items = list(industry_map.values())
        industry_items = sorted(industry_items, key=lambda x: len(x.stocks), reverse=True)
        with open('hour_recommend_stocks.csv', 'w', newline='\n') as f:
            fw = csv.writer(f)
            fw.writerow(['股票代码', '股票名称', '股票市值', '股票行业', '股票概念', '涨幅'])

            for industry_item in industry_items:
                if len(industry_item.stocks) == 1:
                    continue
